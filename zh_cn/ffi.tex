...
\textsl{map\_build} 工具使用正则表达式解析对象声明块，解析过程中首先会匹配出整个对象声明块，并从中匹配出 \texttt{type} 和 \texttt{object\_name} 信息，对 \texttt{attributes} 和 \texttt{data\_fields} 信息则要进一步的解析。为了实现方便，该工具对对象声明块的语法要求并不严格，并且缺少完善的错误处理机制，因此在编写对象声明块时应该确保语法正确。
...
编译期构造的 Map 也就是常量 Map 对象。该类对象的对象声明块使用类型 \texttt{map}。
