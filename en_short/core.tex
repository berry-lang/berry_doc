...
\begin{tabular}{lp{0.78\columnwidth}}
    ...
    \textsf{number} & 0 is converted to \texttt{false}, others are converted to \texttt{true}. \\
...
\hangpar \texttb{for} \textib{id} \texttt{=} \textib{expr}\texttt{,} \textib{cond}[\texttt{,} \textib{expr}] \textib{block} \texttb{end}\quad loop for statement, (not support now).
...
\{\texttb{var} \textib{id}\{\texttt{,} \textib{id}\} | \hangpar \texttb{def} \textib{id} \texttt{(}\textib{args}\texttt{)} \textib{block} \texttb{end}\}
...
\begin{tabular}{@{}lp{0.53\columnwidth}}
    \texttt{r=0..5} & New range from \texttt{0} to \texttt{5}. \\
\end{tabular}

\subsection*{Exception handling}
\hangpar \texttb{throw} \textib{exception} [\texttt{,} \textib{message}]\\
Thorw a \textib{exception} value and unnecessary \textib{message} value.
...
\} \texttb{end}
\begin{quote}
    One or more \texttb{except} blocks must exist. Only runtime exceptions can be catch.
\end{quote}
