...
\hangpar \href{https://berry.readthedocs.io/en/latest/source/en/Reference.html}{Full documentation}
...
\begin{tabular}{@{}llllll}
    ...
    \texttt{\textbackslash uXXXX} & \multicolumn{5}{l}{Unicode character (UTF-8 encoded).}
\end{tabular}\\

\hangpar String indexing and slicing:

\begin{tabular}{@{}lp{0.70\columnwidth}}
\texttt{s[0]} & First character. \\
\texttt{s[-1]} & Last character. \\
\texttt{s[1..3]} & Characters from index 1 to 3. \\
\texttt{s[1..]} & Characters from index 1 to end. \\
\texttt{s[0..-2]} & All characters except the last one. \\
\end{tabular}\\

\hangpar String concatenation:

\begin{tabular}{@{}lp{0.70\columnwidth}}
\texttt{"a" + "b"} & Concatenate with + operator. \\
\texttt{"a" .. "b"} & Concatenate with .. operator. \\
\texttt{"a" "b" "c"} & Adjacent string literals are automatically concatenated. \\
\end{tabular}\\

\hangpar String repetition:

\begin{tabular}{@{}lp{0.70\columnwidth}}
\texttt{"aze" * 3} & Repeat string 3 times: \texttt{"azeazeaze"}. \\
\texttt{"aze" * 0} & Empty string: \texttt{""}. \\
\texttt{"aze" * true} & String if true: \texttt{"aze"}. \\
\texttt{"aze" * false} & Empty string if false: \texttt{""}. \\
\end{tabular}\\
...
Static variables can be accessed via \texttt{self} in instance methods (e.g., \texttt{self.a} for \texttt{static var a}).\\
Static methods have an implicit \texttt{\_class} argument to access the class object and other static members (e.g., \texttt{\_class.a}).
...
\hangpar Range-based access (slicing):

\begin{tabular}{@{}lp{0.70\columnwidth}}
\texttt{l[1..3]} & Elements from index 1 to 3 inclusive. \\
\texttt{l[1..]} & Elements from index 1 to end. \\
\texttt{l[1..-1]} & Elements from index 1 to last element. \\
\texttt{l[0..-2]} & All elements except the last one. \\
\texttt{l[-2..-1]} & Last two elements. \\
\end{tabular}
...
\hangpar \texttb{raise} \textib{exception} [\texttt{,} \textib{message}]\\
Raise a \textib{exception} value and optional \textib{message} value.
...
\begin{enumerate}
    \item \textbf{Exception Handling}: Using \texttt{try}/\texttt{except} blocks to catch and handle exceptions.
    \item \textbf{Return Value Checking}: Many functions return special values to indicate errors.
\end{enumerate}

\hangpar Examples of return value checking:\\
\begin{tabular}{@{}lp{0.70\columnwidth}}
\texttt{json.load('...')} & Returns \texttt{nil} on parsing error. \\
\texttt{list.find(item)} & Returns \texttt{nil} if item is not present. \\
\texttt{map.find("key")} & Returns \texttt{nil} if key doesn't exist. \\
\texttt{string.find("foo", "z")} & Returns \texttt{-1} if substring not found. \\
\end{tabular}
