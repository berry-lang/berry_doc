...
Raises \texttt{'assert\_failed'} when \textib{expr} is \textbf{false}, and \textib{msg} is an optional exception message.
...
Get the superclass of a class \textib{object} or the sub-instance of instance \textib{object} from its superclass. The \textib{object} is a class or an instance.
...
\hangpar \href{https://berry.readthedocs.io/en/latest/source/en/Chapter-7.html\#list-class}{Full documentation}
...
\hangpar \href{https://berry.readthedocs.io/en/latest/source/en/Chapter-7.html\#map-class}{Full documentation}
...
Get the value mapped by the \textib{key}. It will raise a \texttt{"key\_error"} exception when the key-value pair does not exist.
...
\hangpar \href{https://berry.readthedocs.io/en/latest/source/en/Chapter-7.html\#range-class}{Full documentation}
...
\hangpar \href{https://berry.readthedocs.io/en/latest/source/en/Chapter-7.html\#string-module}{Full documentation}
...
\hangpartwo \texttt{string.startswith(}\textib{s}\texttt{,} \textib{sub}[\texttt{,} \textib{case\_insensitive}]\texttt{)}\\
\texttt{string.endswith(}\textib{s}\texttt{,} \textib{sub}[\texttt{,} \textib{case\_insensitive}]\texttt{)}\\
Check whether the string \textib{s} starts/ends with the substring \textib{sub}; case-insensitive if \textib{case\_insensitive} is \texttt{true}.
...
\hangpartwo \texttt{string.toupper(}\textib{text}\texttt{)}\\
\texttt{string.tolower(}\textib{text}\texttt{)}\\
Convert the \textib{text} to uppercase or lowercase; ASCII only no support for Unicode.

\hangpar \texttt{string.tr(}\textib{text}\texttt{, }\textib{chars}\texttt{, }\textib{char\_or\_empty\_string}\texttt{)}\\
Replaces in \textib{text} any occurrence of character(s) from \textib{chars} to a single character, or remove if empty string.

\hangpar \texttt{string.replace(}\textib{text}\texttt{, }\textib{text1}\texttt{, }\textib{text2}\texttt{)}\\
Replaces in \textib{text} occurrence of \textib{text1} with \textib{text2} (this is slower than \texttt{string.tr()}
...
\hangpartwo \texttt{string.format(}\textib{fmt}[\texttt{,} \textib{args}]\texttt{)}\\
\texttt{format(}\textib{fmt}[\texttt{,} \textib{args}]\texttt{)}\\
...
\hangpar \href{https://berry.readthedocs.io/en/latest/source/en/Chapter-7.html\#math-module}{Full documentation}
...
\hangpar \texttt{math.round(}\textib{value}\texttt{)} \\
Return the rounded \textib{value} as \texttt{real}, rounded down for positive and up for negative.
...
Return the minimum/maximum value among all \textib{values}, as \texttt{int} or as \texttt{real} any argument is \texttt{real}.
...
\hangpar \texttt{math.isinf(}\textib{value}\texttt{)} \\
Return \texttt{true} if \textib{value} is a \texttt{inf} or \texttt{-inf}.
...
\hangpar \href{https://berry.readthedocs.io/en/latest/source/en/Chapter-7.html\#bytes-class}{Full documentation}
...
Read or write byte at \textib{index} as \texttt{int}; raises an exception if index is out of bounds.
...
\hangpar \textib{bytes}\texttt{.set(}\textib{offset}\texttt{, }\textib{value}\texttt{[, }\textib{size}\texttt{])} \\
Set the value at \textib{offset} as an unsigned integer of \textib{size} bytes (default 1) with \textib{value} (\textib{size} can be $1$, $2$, $3$, $4$ for Little Endian or $-2$, $-3$, $-4$ for Big Endian). No effect if indices are out of bounds.

\hangpar \textib{bytes}\texttt{.seti(}\textib{offset}\texttt{, }\textib{value}\texttt{[, }\textib{size}\texttt{])} \\
...
\hangpar \href{https://berry.readthedocs.io/en/latest/source/en/Chapter-7.html\#module-global}{Full documentation}
...
\hangpartwo \texttt{global.member(}\textib{id}\texttt{)}\\
\texttt{global.}\textib{id} \\	
...
\hangpar \href{https://berry.readthedocs.io/en/latest/source/en/Chapter-7.html\#json-module}{Full documentation}

\hangpar \texttt{json.load(}\textib{string}\texttt{)} \\
Parse \textib{string} as JSON and convert to Berry objects. Returns \texttt{nil} if parsing fails.
...
\hangpar \href{https://berry.readthedocs.io/en/latest/source/en/Chapter-7.html\#module-introspect}{Full documentation}
...
\hangpar \href{https://berry.readthedocs.io/en/latest/source/en/Chapter-7.html\#module-os}{Full documentation}
