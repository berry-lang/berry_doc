...
\hangpar \texttt{f"}\textib{f\_fmt}\texttt{"}

\hangpar \texttt{f'}\textib{f\_fmt}\texttt{'}\\

\textib{f\_fmt} is a template string in which each value is surrounded by \texttt{\{ \}}
...
Convert \textib{bytes} buffer to a hex \texttt{string}.

\hangpar \textib{bytes}\texttt{.tohex(}\textib{string}\texttt{)}\\
