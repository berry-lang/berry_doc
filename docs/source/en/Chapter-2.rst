...
``if`` statements can use any type of parameters. The rules for converting various types to Boolean types are:

-  ``nil``: converts to ``false``
-  **Integer**: ``0`` converts to ``false``, all other values convert to ``true``
-  **Real number**: ``0.0`` converts to ``false``, all other values convert to ``true``
-  **String**: empty string ``""`` converts to ``false``, all other strings convert to ``true``
-  **Bytes**: empty buffer converts to ``false``, non-empty buffer converts to ``true``
-  **Comobj** and **Comptr**: ``NULL`` pointer converts to ``false``, all other values convert to ``true``
-  **Instance**: if the instance has a ``tobool()`` method, its return value is used; otherwise converts to ``true``
